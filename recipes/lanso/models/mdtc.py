...
class MDTC(nn.Module):
    ...
    def __init__(
        ...
        self.fc = nn.Linear(res_channels,out_channels);
    ...
    def normalize_length_causal(self, skip_connections: list):
        ...
        for x in skip_connections:
            ...
            # print('remove_length:{}'.format(remove_length))#/120/60/0
            if remove_length != 0:
    ...
    def forward(self, x: torch.Tensor):
        ...
        outputs = torch.nn.functional.log_softmax(outputs, dim=-1) # for nl1_loss
        # x = F.softmax(x[:, 0, :], dim=-1)  # export model

        return outputs


if __name__ == '__main__':
    # mdtc = MDTC(3, 4, 40, 32, 5, causal=True)
    mdtc = MDTC(3, 4, 40, 32, 4)
    print(mdtc)

    num_params = sum(p.numel() for p in mdtc.parameters())
    print('the number of model params: {}'.format(num_params))
    x = torch.zeros(128, 151, 40)  # batch-size * time * dim
