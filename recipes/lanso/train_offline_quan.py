...
class SpeakerBrain(sb.core.Brain):
    ...
    def __init__(  # noqa: C901
        ...
        m = F.load_as_dict("/home/wangwei/work/kws/kws_lanso/bin/tdnn4_save22_seed1081_latest_quan.mnn")
        ...
        config_file = "hparams/config.yaml"
        f = open(config_file)
        ...
        self.mnn_net = nn.load_module(inputs, outputs, True)

        # no use optimizer
        self.opt = MNN.optim.SGD(self.mnn_net, 0.01, 0.9, 0.0005)

        nn.compress.train_quant(self.mnn_net, quant_bits=8)
    ...
    def compute_forward(self, batch, stage):
        ...
        feats_mnn = feats.unsqueeze(1).detach().cpu().numpy()
        # print("................feats:{}................".format(feats_mnn.shape))
        if feats_mnn.shape[0] == 128:
            dv = F.const(feats_mnn.flatten().tolist(), [128, 1, 151, 40], F.data_format.NCHW, F.dtype.float)
            predicts = self.mnn_net.forward([dv])
            # fake update
            self.opt.step(F.const(0.0, []))
            for predict in predicts:
                predict.read()
        ...
        * ``update_average()``
        ...
        for epoch in epoch_counter:
            ...
            if valid_set is not None:
                ...
                predicts = self.mnn_net.forward(self.input_placeholders)
                print("quantized model save to " + "quan.mnn")
                F.save(predicts, "quan.mnn")
                avg_valid_loss = 0.0
                with torch.no_grad():
                    for batch in tqdm(
