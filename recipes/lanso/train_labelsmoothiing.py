...
class SpeakerBrain(sb.core.Brain):
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        # print('.......predictions[:,0,:]:{}'.format(predictions[:,0,:].shape))
        # loss = torch.nn.MSELoss()(predictions[:,0,:], torch.nn.functional.one_hot(command, num_classes=3)[:,0,:].to(torch.float32))
        loss = self.loss_func(predictions, command)
